...
block_mv() {
  ...
  make_tmp; row="$REPLY/row"
  printf '%s\t%s\t%s\n' "$old" "$new" "$(date +%Y-%m-%d)" | tee -a "$PR_WT/moves.tsv" > "$row"
  while IFS= read -r f; do rewrite_moves "$row" "$f" || true; done <<EOF
...
EOF
  ...
  pr_finish "delphi: move $old -> $new" "Moves \`$old\` to \`$new\` and rewrites references. Workspaces follow on their next refresh."
  printf '%s\n' "$PR_BRANCH"
}
