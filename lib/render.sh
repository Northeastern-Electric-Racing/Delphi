# render.sh — layout -> workspace files + .delphi/lock.tsv (segment map). Deterministic.
#
# render <src-root> <layout-dir> <outdir>
...
_r_file() {
  ...
  [ -f "$src" ] || die "render: missing file context/$2"
  [ -s "$src" ] || die "render: empty file context/$2"
  mkdir -p "$(dirname "$out")"
...
_r_copy() {
  [ -e "$R_OUT/$1" ] && die "render: two sources map to the same output '$1'"
  _r_file "$1" "$2"
}
...
_r_expand() {
  ...
  case $1 in
    */\*)
      ...
      [ -d "$abs" ] || die "render: no such directory context/$dir"
      n=0
      for f in "$abs"/*; do [ -f "$f" ] && { printf '%s/%s\n' "$dir" "${f##*/}"; n=$((n + 1)); }; done
      [ "$n" -gt 0 ] || die "render: glob matches nothing: $1"
      ;;
    *) printf '%s\n' "$1" ;;
  esac
}
...
_r_header() {
  printf '%s\n' \
    "<!-- Rendered by Delphi from layout '$1'. Edit freely: every line is traced to its source block. -->" \
...
render() {
  ...
  mkdir -p "$R_OUT/.delphi" && : > "$R_LOCK" || die "render: cannot write $R_OUT"
  ...
  [ -n "$R_HARNESS" ] || die "render: $layout/manifest.yml has no harness"
...
EOF
  ...
  while IFS= read -r item; do
    ...
    while IFS= read -r f; do
      case "/$f" in */blocks/*) ;; *) die "render: '$f' is listed under blocks: but is not in a blocks/ directory" ;; esac
      _r_copy "context/$f" "$f"
    done <<EOF2
$files
EOF2
  ...
  while IFS= read -r item; do
    ...
    case $item in
      *.skill)
        ...
        [ -n "$sname" ] && [ -n "$sdesc" ] || die "render: $item needs name and description"
        skdir="$HARNESS_SKILLS_DIR/$sname"
        [ -e "$R_OUT/$skdir" ] && die "render: two skills named '$sname'"
...
EOF2
      ...
      *)
        ...
        [ -f "$abs/SKILL.md" ] || die "render: skill '$item' has no SKILL.md"
        skdir="$HARNESS_SKILLS_DIR/${item##*/}"
        [ -e "$R_OUT/$skdir" ] && die "render: two skills named '${item##*/}'"
